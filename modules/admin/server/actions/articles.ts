...
export async function createArticle(_prev: ArticleActionResult | null, formData: FormData): Promise<ArticleActionResult> {
  try {
    ...
    const featuredOnHome = formData.get("featuredOnHome") === "on" || formData.get("featuredOnHome") === "true";
    ...
    const coverObjectPosition = normalizeCoverObjectPosition(parsed.data.coverObjectPosition ?? "50% 50%");
